...
print("Loading RoBERTa modle...", end='')
tokenizer = RobertaTokenizer.from_pretrained("roberta-base")
model = RobertaModel.from_pretrained("roberta-base")
print("Done")

# Function to generate RoBERTa embeddings for sentences
def create_roberta_embeddings(sentences):
...
# Update your embedding function to use RoBERTa
def process_sentence(sentence_pair):
    sentence_id, sentence = sentence_pair
    embedding = create_roberta_embeddings([sentence])
    normalized_embedding = embedding / np.linalg.norm(embedding)
    return sentence_id, normalized_embedding[0]
...
def main():
    ...
    # Embeding BDI 
    # Update the function where you embed the BDI tokens
    BDI_embeddings = []
    for bsentence in tqdm(BDI_list, desc="Embedding BDI tokens"):
        sentence_embedding = create_roberta_embeddings([bsentence])
        normalized_embedding = sentence_embedding / np.linalg.norm(sentence_embedding)  # Use numpy for normalization
        BDI_embeddings.append(normalized_embedding)

    # Update the embedding sentences section to use RoBERTa
    print("Embedding sentences...", end='')
